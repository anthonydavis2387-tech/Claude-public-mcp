...
**Voice quality:** the Web Speech API only exposes whatever voices the visitor's OS/browser ships — there's no way to reach a higher-quality TTS provider (ElevenLabs, etc.) from inside the Artifact sandbox, since that would mean a client-side fetch to a third-party host the CSP doesn't allow (and exposing an API key client-side besides). Do the best that's actually possible instead: default `rate` to ~0.95 and `pitch` to ~0.85 (a touch slower and lower reads as smoother and less robotic than the browser's un-tuned default), and rank `synth.getVoices()` to prefer names that tend to be natural-sounding male voices across platforms (David, Guy, Mark, Ryan, Daniel, Alex, Tom, Fred, Gordon, Oliver — plus a bonus for "Natural"/"Enhanced"/"Premium"/"Neural" in the name, which usually flags a higher-quality voice build) over the OS default. Expose a "Voice settings" panel (voice picker + rate/pitch sliders + a preview button) so the user can override the guess for their own device — voice availability is entirely device-dependent, so the auto-pick is a best guess, not a guarantee. Persist the user's choice to `localStorage` (per-viewer, wrapped in try/catch per the artifact-design rules) so it survives a page reload; it can't be read back by Claude or shared across the user's devices, so don't treat it as durable state the next session can assume. Also wire an optional voice-command toggle via `SpeechRecognition`/`webkitSpeechRecognition` (feature-detect and disable the mic button with an explanatory `title` if unsupported — this is Chrome/Edge/Safari-only, not Firefox, and can fail silently if the Artifact's embedding context blocks microphone permission; the read-aloud buttons must work regardless of whether voice commands do). Match simple keyword commands (bitcoin/btc, flag, next, brief/dashboard/review, stop) rather than requiring exact phrasing. **The spoken text is hand-written per section to read naturally (numbers spelled out, "%" read as "percent," no markup) — it is not scraped from the DOM.** That means it goes stale exactly like every other section: regenerate all four spoken strings from that session's actual numbers every time the dashboard is rebuilt, the same as the visible text. Keep each section's spoken text short enough to actually listen to (a paragraph or two, not a full read of every row).

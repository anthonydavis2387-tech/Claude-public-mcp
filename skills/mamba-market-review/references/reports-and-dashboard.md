...
**Voice quality:** the Web Speech API only exposes whatever voices the visitor's OS/browser ships — there's no way to reach a higher-quality TTS provider (ElevenLabs, etc.) from inside the Artifact sandbox, since that would mean a client-side fetch to a third-party host the CSP doesn't allow (and exposing an API key client-side besides). Do the best that's actually possible instead: default `rate` to ~0.95 and `pitch` to ~0.85 (a touch slower and lower reads as smoother and less robotic than the browser's un-tuned default).

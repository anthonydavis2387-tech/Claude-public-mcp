...
Preferred approach: use FMP's stock screener endpoint (`/stable/company-screener` or `/api/v3/stock-screener`), filtered to `exchange=NASDAQ,NYSE`, **`country=US`**, `isEtf=false&isFund=false&isActivelyTrading=true`, sorted by `marketCap` descending, limit 100. **The `country=US` filter is not optional** — confirmed live that omitting it lets ~44 foreign-domiciled companies (TSM, BABA, HSBC, SAP, Toyota, Shell, Novo Nordisk, HDFC Bank, and similar large NYSE/NASDAQ-listed but non-U.S. issuers) into the results as if they were large-cap U.S. constituents, which is wrong for both a "Russell proxy" and an "S&P 500" framing — both indices are U.S.-domestic by construction. Also sanity-check for duplicate tickers resolving to the same company name (confirmed live: `SOJE` and `SOMN` both resolved to "The Southern Company," alongside the real `SO` ticker — drop the duplicates, keep the shortest/primary symbol) before finalizing the list. Cross-reference against Dow 30, Nasdaq 100, and S&P 500 Top 100 from above and drop duplicates (Step 2 in SKILL.md).
